...
class MultimodalAADBDataset(Dataset):
    """
    多模态AADB数据集：同时返回视觉特征和文本特征
    """
    ...
    def __getitem__(self, item):
        ...
        # AADB数据格式：score已经是归一化后的0-1
        y = np.array([row['score']])
        # AADB数据格式：使用filename列
...
def test_multimodal_dataset():
    """测试多模态数据集"""
    # 配置路径 (AADB)
    csv_path = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/AADB/datasets/train.csv"
    images_path = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/AADB/images/"
    text_features_root = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/AADB/text_features/"
    
    try:
        dataset = MultimodalAADBDataset(csv_path, images_path, text_features_root, if_train=False)
