...
print(f"\n{'='*80}")
print(f"🚀 HKD-IAA AGREE训练脚本 (AADB, 不含HMAI指标)")
...
class ErrorTrackerV2:
    """困难样本加权：warmup + mean(weights)=1（不引入HMAI指标）"""
...
def create_data_part(opt):
    ...
    train_dataset = MultimodalAADBDataset(os.path.join(csv_root, 'train.csv'), images, text_root, if_train=True)
    val_dataset = MultimodalAADBDataset(os.path.join(csv_root, 'test.csv'), images, text_root, if_train=False)
