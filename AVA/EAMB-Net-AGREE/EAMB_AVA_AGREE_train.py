...
print(f"\n{'='*80}")
print(f"🚀 EAMB-Net AGREE训练脚本 - AVA (不含HMAI指标)")
...
def get_ava_score(y_dist, device):
    """AVA评分：分布加权求和 (1-10)"""
    w = torch.from_numpy(np.linspace(1, 10, 10)).type(torch.FloatTensor).to(device)
    w_batch = w.repeat(y_dist.size(0), 1)
    return (y_dist * w_batch).sum(dim=1)


def emd_loss_per_sample(p_target, p_estimate):
    """EMD loss（返回每个样本的loss向量）"""
    cdf_target = torch.cumsum(p_target, dim=1)
    cdf_estimate = torch.cumsum(p_estimate, dim=1)
    cdf_diff = cdf_estimate - cdf_target
    return torch.mean(torch.abs(cdf_diff), dim=1)
...
def create_data_part(opt):
    ...
    train_ds = MultimodalAVADataset(os.path.join(csv_root, 'train.csv'), images_path, text_root, if_train=True)
    val_ds = MultimodalAVADataset(os.path.join(csv_root, 'val.csv'), images_path, text_root, if_train=False)
...
def train(opt, model, loader, optimizer, device, epoch=0):
    model.train()
    ...
    for batch in tqdm(loader):
        ...
        if SAGA_VERSION == 'v3':
            ...
            per_sample_emd = emd_loss_per_sample(y, y_pred)
            score_loss = per_sample_emd.mean()
            ...
            per_sample_emd = emd_loss_per_sample(y, y_pred)  # [B]

            if use_error_aware:
                # 权重依据：score误差（1-10加权求和），更符合“难样本”定义
                pred_scores = get_ava_score(y_pred, device)
                true_scores = get_ava_score(y, device)
                sample_errors = torch.abs(pred_scores - true_scores)  # [B]
                ...
                score_loss = (weights * per_sample_emd).mean()
                ...
                if ERROR_AWARE_ENABLED and error_tracker is not None:
                    pred_scores = get_ava_score(y_pred, device)
                    true_scores = get_ava_score(y, device)
                    sample_errors = torch.abs(pred_scores - true_scores).detach()
...
def validate(opt, model, loader, device):
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            loss = emd_loss_per_sample(y, y_pred).mean()
            validate_losses.update(loss.item(), x.size(0))

            pred_score_values = get_ava_score(y_pred, device)
            true_score_values = get_ava_score(y, device)
            pred_scores += pred_score_values.data.cpu().numpy().tolist()
            true_scores += true_score_values.data.cpu().numpy().tolist()
...
def start_train(opt):
    ...
    for epoch in range(start_epoch, opt['num_epoch']):
        ...
        train_loss = train(opt, model, train_loader, optimizer, device, epoch=epoch)
        val_loss, vacc, vsrcc, vlcc = validate(opt, model, val_loader, device)
