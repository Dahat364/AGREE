...
class MultimodalAVADataset(Dataset):
    """
    多模态AVA数据集：同时返回视觉特征和文本特征
    AVA数据集特点：10维评分分布（score2-score11）
    """
    ...
    def __getitem__(self, item):
        ...
        # AVA数据格式：10维评分分布 (score2-score11)
        ...
        # AVA数据格式：image_id列已包含.jpg后缀
        image_id = str(row['image_id'])  # 例如：'953417.jpg'
...
def test_multimodal_dataset():
    """测试多模态数据集"""
    # 配置路径 (AVA)
    csv_path = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/AVA/AVA_dataset/AVAdata/train.csv"
    images_path = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/AVA/AVA_dataset/image/"
    text_features_root = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/AVA/AVA_dataset/text_features/"
    
    try:
        dataset = MultimodalAVADataset(csv_path, images_path, text_features_root, if_train=False)
