...
print(f"\n{'='*80}")
print(f"🚀 HKD-IAA AGREE训练脚本 - AVA (不含HMAI指标)")
print(f"   基础版本: SAGA-{SAGA_VERSION}")
...
def create_data_part(opt):
    ...
    train_dataset = MultimodalAVADataset(os.path.join(csv_root, 'train.csv'), images, text_root, if_train=True)
    val_dataset = MultimodalAVADataset(os.path.join(csv_root, 'test.csv'), images, text_root, if_train=False)

    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=opt['batch_size'], num_workers=opt['train_num_workers'], shuffle=True)
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=opt['batch_size'], num_workers=opt['test_num_workers'], shuffle=False)
...
def get_ava_score_np(y_dist_np):
    weights = np.arange(1, 11).astype('float32')
    return (y_dist_np * weights).sum(axis=1)


def get_ava_score_torch(y_dist, device):
...
def train(opt, model, loader, criterion, device, epoch=0):
    model.train()
    ...
    for batch in tqdm(loader):
        ...
        if use_error_aware:
            # 权重依据：score误差（1-10加权求和）
            pred_scores = get_ava_score_torch(y_pred, device)
            true_scores = get_ava_score_torch(y, device)
            ...
            # 加权损失：基于分布MSE（每样本 sum over bins）
            sample_losses = (y_pred - y).pow(2).sum(dim=1)  # [B]
...
def validate(opt, model, loader, criterion, device):
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            pred_scores += get_ava_score_np(pscore_np).tolist()
            true_scores += get_ava_score_np(tscore_np).tolist()
...
def start_train(opt):
    ...
    for e in range(start_epoch, opt['num_epoch']):
        ...
        train_loss = train(opt, model, train_loader, criterion, device, epoch=e)
