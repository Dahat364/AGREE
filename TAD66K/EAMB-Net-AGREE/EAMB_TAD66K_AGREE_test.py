...
def create_test_loader(opt):
    ...
    dataset = MultimodalTAD66KDataset(
        test_csv_path,
        opt['path_to_images'],
...
def evaluate(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            pred_scores += y_pred.cpu().numpy().mean(axis=1).tolist()
            true_scores += y.cpu().numpy().mean(axis=1).tolist()
    ...
    rmse = np.sqrt(mse)
    acc = accuracy_score(
        (true_scores > 0.55).astype(int),
        (pred_scores > 0.55).astype(int)
    )
