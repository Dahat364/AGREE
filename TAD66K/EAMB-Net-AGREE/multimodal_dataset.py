...
class MultimodalTAD66KDataset(Dataset):
    """
    多模态TAD66K数据集：同时返回视觉特征和文本特征
    """
    ...
    def __getitem__(self, item):
        ...
        # TAD66K数据格式：score范围0-10，归一化到0-1
        y = np.array([row['score'] / 10])
        # TAD66K数据格式：直接使用image列
        image_id = row['image']
...
def test_multimodal_dataset():
    """测试多模态数据集"""
    # 配置路径
    csv_path = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/TAD66K/labels/merge/processed/train.csv"
    images_path = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/TAD66K/images/"
    text_features_root = "/home/dmc/12TB_ZHZ68JLF/A_PROJECT_IAA/IAADatasets/TAD66K/text_features/"
    
    try:
        dataset = MultimodalTAD66KDataset(csv_path, images_path, text_features_root, if_train=False)
